...
@app.get("/counterparty/sql-failed-records")
def get_counterparty_sql_failed_records() -> list[dict[str, Any]]:
    """
    Return the failed records generated by the SQL quality checks.
    """
    df = read_csv_report("sql_counterparty_trade_failed_records.csv")
    return dataframe_to_records(df)
...
@app.get("/counterparty/curation-summary")
def get_counterparty_curation_summary() -> dict[str, Any]:
    """
    Return a compact summary of the curated versus quarantined outputs.
    """
    ...
    if not curated_path.exists() or not quarantine_path.exists():
        raise HTTPException(
            status_code=404,
            detail=(
                "Counterparty curation outputs were not found. "
                "Run `python -m entityq.run_full_demo` first."
            ),
        )
    ...
    total_records = len(curated) + len(quarantine)

    return {
        "total_records": total_records,
        "curated_records": len(curated),
        "quarantine_records": len(quarantine),
        "curated_rate_pct": round((len(curated) / total_records) * 100, 2)
        if total_records
        else 0,
        "quarantine_rate_pct": round((len(quarantine) / total_records) * 100, 2)
        if total_records
        else 0,
